...
public class AllResources extends SelfCustomLog {
    private Context mC;
    ...
    public AllResources(Context mC, AllAbilities allAbilities, AllCapacities allCapacities) throws Exception {
        this.mC = mC;
        ...
        try {
            ...
            reset();
            throw new AllAttributeError("Error during AllResources creation", e);
        }
    }

    private void buildResourcesList() throws Exception {
    ...
    public String readValue(String tag, Element element) {
        try {
            ...
            return ""; //some resource do'nt have allfiels (for example hide, testable etc)
        }
    }
    ...
    public boolean checkAnyConvertibleAvailable() {
        ...
        for (int rankConv = 1; rankConv <= 6; rankConv++) {
            if (getResource("spell_conv_rank_" + rankConv).getCurrent() > 0) {
                bool = true;
                break;
            }
        }
    ...
    public void resetRessources() throws Exception {
        buildResourcesList();
    }
