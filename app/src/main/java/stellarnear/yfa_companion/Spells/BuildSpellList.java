...
public class BuildSpellList extends SelfCustomLog {
    ...
    public static BuildSpellList getInstance(Context mC) throws SpellError {  //pour eviter de relire le xml à chaque fois
        if (instance == null) {
    ...
    private BuildSpellList(Context mC) throws SpellError {  // on construit la liste qu'une fois dans MainActivityFragmentSpell donc pas besoin de singleton
        try {
            this.allSpells = new SpellList();
            addSpells(mC, "");
            addSpells(mC, "Mythic");
        } catch (Exception e) {
            throw new SpellError("Error during BuildSpellList creation", e);
        }
    }
